...
This note captures the minimal set of changes/images needed to get a full Netlab→Clabernetes→Ansible E2E working with Arista cEOS when using `connectivity: multus` and `native mode`.
...
- Netlab ansible runner image was outdated and assumed `hosts.yml` had `all.hosts`.
- Arista cEOS SSH was not enabled by default for Netlab Ansible readiness.

## Current working images (prod/qa)

- `ghcr.io/forwardnetworks/skyforge-server:20260118-sshfix-3b53fce`
  - Includes Skyforge-side injection to enable `management ssh` on cEOS nodes for netlab-c9s.
...
- `ghcr.io/forwardnetworks/skyforge-netlab-ansible-runner:20260118-ansiblefix-3b53fce`
  - Patches `hosts.yml` by recursively finding `hosts:` maps (no longer assumes `all.hosts`).
