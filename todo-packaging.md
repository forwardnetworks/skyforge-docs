...
- [ ] Add `scripts/deploy.sh` that:
  ...
  - runs smoke checks (`SKYFORGE_SMOKE_SCOPE=deploy-forward ./scripts/post-deploy-smoke.sh`) after the rollout
...
- [ ] Add CI job(s) to run:
  ...
  - template validators:
    ...
    - `(cd server && go run ./cmd/validatekne --root ../blueprints/kne)`
